          ...
          child: InteractiveViewer(
            child: GestureDetector(
              onTapUp: (details) {
                ...
                if (row >= 0 &&
                    row < _grid.navGrid.length &&
                    col >= 0 &&
                    col < _grid.navGrid[row].length) {
                  setState(() {
                    _grid.navGrid[row][col] = !_grid.navGrid[row][col];
                  });
...
class _NavigationPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int row = 0; row < navGrid.length; row++) {
      for (int col = 0; col < navGrid[row].length; col++) {
